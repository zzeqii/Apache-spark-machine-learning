...
FILE_FLAG = '-s1-d50-k5-e8-c1'
OUTPUT_DIR = 'hdfs://soit-hdp-pro-1.ucc.usyd.edu.au/user/anli9209'
...
predictions = test_data.rdd.map(lambda x: (x.test_id, make_prediction(
    np.array(x.test_feature), features, labels, K=K_NEAREST_NEIGHBORS))).collect()
...
overall_report(test_data, prediction_data)
# overall_metrics.show()

# # Calculate class level statistics
classification_report(test_data, prediction_data)
